...
@_silgen_name("daytona2_pixels") private func nativePixels(_ context: UnsafeMutableRawPointer) -> UnsafePointer<UInt8>?
@_silgen_name("daytona2_audio") private func nativeAudio(_ context: UnsafeMutableRawPointer) -> UnsafePointer<Int16>?
@_silgen_name("daytona2_audio_count") private func nativeAudioCount(_ context: UnsafeMutableRawPointer) -> Int32
@_silgen_name("daytona2_width") private func nativeWidth(_ context: UnsafeMutableRawPointer) -> Int32
@_silgen_name("daytona2_height") private func nativeHeight(_ context: UnsafeMutableRawPointer) -> Int32
@_silgen_name("daytona2_frame_rate") private func nativeFrameRate(_ context: UnsafeMutableRawPointer) -> Double
@_silgen_name("daytona2_audio_sample_rate") private func nativeSampleRate(_ context: UnsafeMutableRawPointer) -> Int32
@_silgen_name("daytona2_frame_number") private func nativeFrameNumber(_ context: UnsafeMutableRawPointer) -> UInt64

enum DaytonaPreferences {
    static let domain = "local.william.daytonausa2"
...
final class DaytonaGame {
    struct Frame {
        ...
        func writePNG(to url: URL) throws {
            guard let image, let destination = CGImageDestinationCreateWithURL(url as CFURL, UTType.png.identifier as CFString, 1, nil) else {
                throw DaytonaUSA2Error.message("Cannot create capture at \(url.path).")
            }
    ...
    init() throws {
        ...
        guard let pointer else {
            ...
            throw DaytonaUSA2Error.message(nativeError(nil).map { String(cString: $0) } ?? "The Daytona USA 2 native engine could not initialize.")
        }
        ...
        guard width == 496, height == 384, fps.isFinite, (1...240).contains(fps), (8_000...192_000).contains(rate) else {
            ...
            throw DaytonaUSA2Error.message("The native engine returned an unsupported Model 3 video or audio format.")
        }
    ...
    private func requireOpen() throws {
        guard !closed else { throw DaytonaUSA2Error.message("The game session has closed.") }
    }
    private var failure: DaytonaUSA2Error {
    ...
    func advance(_ input: DaytonaInput) throws -> [Int16] {
        ...
        guard nativeStep(context, input.steering, input.accelerator, input.brake, input.buttons) == 1 else { throw failure }
        guard nativeFaultCode(context) == 0 else { throw failure }
        guard nativeFrameNumber(context) == UInt64(frameCount + 1), let pixels = nativePixels(context) else {
            throw DaytonaUSA2Error.message("The engine did not return the expected completed video frame.")
        }
        ...
        guard (0...8192).contains(count) else { throw DaytonaUSA2Error.message("Invalid native stereo-frame count.") }
        var samples = [Int16]()
        if count > 0 {
            guard let pointer = nativeAudio(context) else { throw DaytonaUSA2Error.message("The engine returned no PCM buffer.") }
...
func runDaytonaReplay(frames: Int, replay: DaytonaReplay?, capture: URL?, trace: URL? = nil) throws -> [String: Any] {
    ...
    for frame in 0..<frames {
        let samples = try game.advance(replay?.input(frame: frame) ?? DaytonaInput())
    ...
    guard let picture = game.latestFrame else { throw DaytonaUSA2Error.message("No frame was produced.") }
    if let capture { try picture.writePNG(to: capture) }
    return ["game": "Daytona USA 2", "frames": frames, "framesPerSecond": game.framesPerSecond,
