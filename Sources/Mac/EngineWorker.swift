...
final class DaytonaEngineWorker {
    struct Presentation {
        let frame: DaytonaGame.Frame?
        let serial: UInt64
        let failure: String?
    }
    private let condition = NSCondition()
    private let inputForFrame: (Int, DaytonaInput?) -> DaytonaInput
    ...
    private var latestFrame: DaytonaGame.Frame?
    ...
    init(inputForFrame: @escaping (Int, DaytonaInput?) -> DaytonaInput) throws {
        self.inputForFrame = inputForFrame
        let thread = Thread { [weak self] in self?.run() }
        self.thread = thread
        thread.name = "Daytona USA 2 engine"
        ...
        let error = failure
        condition.unlock()
        if let error { throw DaytonaUSA2Error.message(error) }
    }

    func attachAudio(_ value: DaytonaAudioOutput) {
    ...
    private func run() {
        do {
            let game = try autoreleasepool { try DaytonaGame() }
            condition.lock()
