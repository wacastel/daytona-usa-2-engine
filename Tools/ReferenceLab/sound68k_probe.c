...
void daytona2_native_fault(const char *message) { fprintf(stderr,"%s\n",message); abort(); }

unsigned daytona2_m68k_board;
static unsigned char rom[2][0x20000];
...
static unsigned memory_byte(unsigned address) {
  ...
  if(daytona2_m68k_board==0 && address>=0x600000 && address<0x700000) {
    unsigned offset=(address-0x600000)&0x7ffff;
    return offset<0x20000 ? rom[0][offset^1] : 0;
  }
  if(daytona2_m68k_board==1 && address<0x20000) return rom[1][address^1];
  if(daytona2_m68k_board==0 && address<16) return rom[0][address^1];
...
void sound_probe_load(unsigned board,const unsigned char *data) { memcpy(rom[board],data,0x20000); }
void sound_probe_run(unsigned board,unsigned pc,unsigned random_seed,uint64_t *result) {
  daytona2_m68k_board=board; seed=random_seed; mutation_count=0;
  ...
  m68k_set_reg(M68K_REG_SP,board==0?0xff00:0xf0ff00);
  m68k_set_reg(M68K_REG_USP,board==0?0xfe00:0xf0fe00);
