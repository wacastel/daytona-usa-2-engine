...
def main():
    ...
    parser.add_argument("--library", type=Path, default=ROOT / "build/native/libdaytona2.dylib")
    parser.add_argument("--route", type=Path, default=ROOT / "Configuration/replays/beginner-auto.json")
        ...
        with tempfile.TemporaryDirectory(prefix="daytona2-sound-lifecycle-") as saves:
            ...
            try:
                for frame in range(args.frames):
                    ...
                    if lib.daytona2_step(context, *state.values()) != 1:
                        raise RuntimeError(f"Session {run + 1} frame {frame + 1}: " + lib.daytona2_error(context).decode())
                    count = lib.daytona2_audio_count(context)
                    pcm = C.string_at(lib.daytona2_audio(context), count * 4)
                    rgba = C.string_at(lib.daytona2_pixels(context), 496 * 384 * 4)
                    audio.update(pcm)
    ...
    result = dict(passed=all(row["differingFrames"] == 0 for row in differences.values()),
                  framesPerSession=args.frames, sessions=2, processRestartBetweenSessions=False,
                  globalStatePatching=False, librarySHA256=library_sha,
