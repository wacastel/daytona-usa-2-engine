...
FILE *output() {
  static FILE *f = []() {
    const char *path = std::getenv("DAYTONA2_SOUND_CAPTURE");
    return path && *path ? std::fopen(path, "a") : nullptr;
  }();
  return f;
}
}
extern "C" void daytona2_sound_observe68k(unsigned board, unsigned pc, unsigned (*read_word)(unsigned)) {
  if (!output()) return;
...
extern "C" void daytona2_sound_observe_dsp(const unsigned short *words) {
  if (!output()) return;
