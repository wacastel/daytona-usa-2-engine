#!/usr/bin/env python3
"""Offline-specialize the supplied Daytona 2 wheel-board Z80 ROM."""
...
from compile_ppc import ROOT, block, exact_replace, sha
...
def main() -> None:
    ...
    rom = (a.game / "epr-20985.bin").read_bytes()
    if len(rom) != 0x10000 or zlib.crc32(rom) != 0xb139481d:
        raise ValueError("Wrong Daytona 2 drive-board ROM")
    ...
    sequences, mapping = [], {}
    address_ids = []
    # The pinned wheel board maps ROM through 0x8fff, then unmapped reads as FF.
    image = rom[:0x9000] + b"\xff" * 3
    bodies = []
    for pc in range(0x9000):
    ...
    generated = ["\n// Fixed ROM addresses, exact four-byte identity guards, no instruction decoder.\n",
                 "static const unsigned char daytona2_z80_words[][4] = {\n"]
    generated += ["{" + ",".join(map(str, sequence)) + "},\n" for sequence in sequences]
    generated.append("};\nstatic const unsigned short daytona2_z80_program[0x9000] = {\n")
    for base in range(0, len(address_ids), 64):
        generated.append(",".join(map(str, address_ids[base:base + 64])) + ",\n")
    generated.append("};\n")
    dispatch = ["  if (pc >= 0x9000) throw std::runtime_error(\"Untranslated Daytona 2 Z80 PC\");\n",
                "  const unsigned fixed_id=daytona2_z80_program[pc];\n",
                "  for (unsigned byte=0;byte<4;++byte) if (GetBYTE(pc+byte)!=daytona2_z80_words[fixed_id][byte])\n",
                "    throw std::runtime_error(\"Changed Daytona 2 Z80 instruction\");\n",
                "  switch(fixed_id) {\n"]
    ...
    manifest = {"format": 1, "cpu": "wheel-board Z80", "romSHA256": hashlib.sha256(rom).hexdigest(),
                ...
                "scope": "All 0x9000 mapped original ROM byte starts; four-byte guards; fixed generated operands; inherited interrupt modes, HALT and prefix behavior."}
