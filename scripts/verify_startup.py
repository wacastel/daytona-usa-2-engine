...
func fixture(_ credits: UInt8 = 13, longComments: Bool = false) -> Fixture {
    let header = block("Daytona2 NVRAM", "Synthetic fixture only", Data())
    var eepromPayload = Data((0..<166).map { UInt8(($0 * 19 + 7) & 255) })
    eepromPayload.replaceSubrange(0..<6, with: [0x33, 0x4d, 0x45, 0x53, 0x41, 0x47])
...
func saved(_ root: URL, _ name: String, _ data: Data) throws -> URL {
    ...
    try data.write(to: directory.appendingPathComponent("daytona2.nv"))
    return directory
}
func contents(_ directory: URL) throws -> Data { try Data(contentsOf: directory.appendingPathComponent("daytona2.nv")) }
func backupURL(_ directory: URL, _ data: Data) -> URL {
    directory.appendingPathComponent("Backups", isDirectory: true)
        .appendingPathComponent("daytona2-before-startup-\(digest(data)).nv")
}
func run() throws {
    let fm = FileManager.default, root = URL(fileURLWithPath: CommandLine.arguments[1], isDirectory: true)
    let domain = "local.william.daytonausa2"
    for args in [[], ["--audio-report", "telemetry.json"], ["--save-dir", "disposable"], ["--capture", "frame.png"]] {
        try require(DaytonaCabinetSave.shouldClearCredits(bundleIdentifier: domain, arguments: args),
                    "Bundled interactive policy admits \(args)")
    }
    for flag in ["--headless", "--self-test", "--diagnostic-run", "--audio-replay"] {
        try require(!DaytonaCabinetSave.shouldClearCredits(bundleIdentifier: domain, arguments: [flag]),
                    "Replay policy preserves \(flag) save")
        try require(!DaytonaCabinetSave.shouldClearCredits(bundleIdentifier: domain, arguments: ["--audio-report", "out.json", flag]),
                    "Replay policy remains excluded with telemetry: \(flag)")
    }
    for identity: String? in [nil, "other.application"] {
        try require(!DaytonaCabinetSave.shouldClearCredits(bundleIdentifier: identity, arguments: []),
    ...
    for count: UInt8 in [1, 13, 24, 255] {
        for longer in [false, true] {
            ...
            let result = try DaytonaCabinetSave.clearingCredits(in: f.data)
            let differences = zip(f.data, result.data).enumerated().filter { $0.element.0 != $0.element.1 }.map { $0.offset }
            try require(result.credits == count && result.data.count == f.data.count && differences == [f.creditOffset],
                        "Counter \(count), variable comments \(longer): exactly the primary credit byte changes")
            try require(result.data[f.creditOffset] == 0 && result.data[f.creditOffset + 1] == 73,
                        "Counter \(count), variable comments \(longer): separate adjacent bank remains 73")
        }
    }
    let f = fixture(), zero = fixture(0)
    let result = try DaytonaCabinetSave.clearingCredits(in: zero.data)
    ...
    let absentCount = try DaytonaCabinetSave.prepareInteractiveLaunch(in: absent)
    ...
    let zeroSave = zeroDir.appendingPathComponent("daytona2.nv"), stamp = Date(timeIntervalSince1970: 1_000_000)
    try fm.setAttributes([.modificationDate: stamp], ofItemAtPath: zeroSave.path)
    let zeroCount = try DaytonaCabinetSave.prepareInteractiveLaunch(in: zeroDir)
    ...
    let normal = try saved(root, "normal", f.data), normalSave = normal.appendingPathComponent("daytona2.nv")
    try fm.setAttributes([.posixPermissions: 0o600], ofItemAtPath: normalSave.path)
    let count = try DaytonaCabinetSave.prepareInteractiveLaunch(in: normal)
    let backup = backupURL(normal, f.data), originalBackup = try Data(contentsOf: backup)
    let changed = try contents(normal), expected = try DaytonaCabinetSave.clearingCredits(in: f.data).data
    ...
    let second = try DaytonaCabinetSave.prepareInteractiveLaunch(in: normal)
    ...
    let reused = try DaytonaCabinetSave.prepareInteractiveLaunch(in: normal)
    ...
    try rejected("Tampered existing backup prevents the save write") { _ = try DaytonaCabinetSave.prepareInteractiveLaunch(in: tampered) }
    ...
    var cases: [(String, Data)] = [
        ...
        ("header-payload", block("Daytona2 NVRAM", "Fixture", Data([1])) + f.eeprom + f.backup),
    ...
    var nameEnd = f.data; nameEnd[12 + "Daytona2 NVRAM".utf8.count] = 65
    ...
    var commentInterior = f.data; commentInterior[12 + "Daytona2 NVRAM".utf8.count + 1] = 0
    cases.append(("interior-comment-nul", commentInterior))
    for (name, data) in cases {
        let directory = try saved(root, name, data)
        try rejected("Reject malformed container: \(name)") { _ = try DaytonaCabinetSave.prepareInteractiveLaunch(in: directory) }
    ...
    try fm.createSymbolicLink(at: links.appendingPathComponent("daytona2.nv"), withDestinationURL: target)
    try rejected("Symbolic-link save is rejected") { _ = try DaytonaCabinetSave.prepareInteractiveLaunch(in: links) }
    ...
    try rejected("Symbolic-link backup directory is rejected") { _ = try DaytonaCabinetSave.prepareInteractiveLaunch(in: linkBackup) }
...
def main():
    ...
    result.update(inputs=inputs, harnessSHA256=sha(harness), executableSHA256=sha(executable),
                  ...
                  scope='Actual Swift credit-cleanup helper and launch-policy tests against disposable synthetic CBlockFile containers. Checks one-byte preservation, exact backups, permissions, idempotence and rejection before writes. NativeGame source is bound for integration review; the engine and GUI are not executed by this fixture. Separate original-program and packaged-startup evidence establishes the game-specific credit meaning and attract behavior.')
