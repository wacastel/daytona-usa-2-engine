#!/usr/bin/env python3
"""Specialize pinned Daytona 2 PowerPC operations and operands offline."""
...
ROM_FILES = [
    ("epr-20864a.20", 0x5250F3A8),
    ("epr-20863a.19", 0x1DEB4686),
    ("epr-20862a.18", 0xE1B2CA61),
...
def verified_rom(game: Path) -> tuple[bytes, list[dict]]:
    ...
    identities = []
    for lane, (name, crc) in enumerate(ROM_FILES):
        path = game / name
        raw = path.read_bytes()
        if len(raw) != 0x200000 or zlib.crc32(raw) != crc:
            raise ValueError(f"Wrong Daytona 2 Revision A ROM: {path}")
        # The pinned loader swaps each source halfword while interleaving four lanes.
        data[lane * 2::8] = raw[1::2]
        data[lane * 2 + 1::8] = raw[0::2]
...
def native_source(source: str, executor: str, operations: str, cpu: Path, out: Path,
    ...
    # Selected CROM program occupies the upper 2 MiB. The game uploads its fixed
    # low-memory image from this region; byte guards authenticate each admitted
    # address even before/after copying. Observed reset stubs and patched words
    # supplement that bounded static image, never runtime discovery.
...
'''
    ...
    for pc, words in variants.items():
        expected = None
        if static_words and (pc < 0x200000 or pc >= 0xffe00000):
            expected = static_words[(pc & 0x1fffff) >> 2]
        extra = sorted(word for word in words if word != expected)
        if extra:
            pages.setdefault(pc >> 16, {})[(pc & 0xffff) >> 2] = extra
    generated.append("struct Daytona2PPCVariant { UINT32 operation; UINT32 next; };\n")
    chains = [(0, 0)]
    for page, entries in sorted(pages.items()):
        ...
        for slot, words in sorted(entries.items()):
            ...
            for word in words:
                ...
                head = len(chains) - 1
            heads[slot] = head
        generated.append(f"static const UINT32 daytona2_ppc_page_{page:04x}[16384] = {{\n")
        for base in range(0, len(heads), 64):
            generated.append(",".join(map(str, heads[base:base + 64])) + ",\n")
        generated.append("};\n")
    generated.append("static const Daytona2PPCVariant daytona2_ppc_variants[] = {\n")
    generated += [f"{{{operation},{nxt}}},\n" for operation, nxt in chains]
    generated.append("};\nstatic inline const UINT32 *daytona2_ppc_page(UINT32 page) {\n switch(page) {\n")
    generated += [f"case 0x{page:04x}: return daytona2_ppc_page_{page:04x};\n" for page in sorted(pages)]
    generated.append("default: return nullptr;\n }\n}\n")
    generated.append("static void daytona2_ppc_fixed(UINT32 pc, UINT32 actual) {\n if (!(pc & 3)) {\n")
    if static_words:
        generated.append(" if (pc < 0x200000 || pc >= 0xffe00000) {\n"
                         "  const auto &entry = daytona2_ppc_operations[daytona2_ppc_program[(pc & 0x1fffff) >> 2]];\n"
                         "  if (entry.run && actual == entry.word) { entry.run(); return; }\n }\n")
    generated.append(" if (const UINT32 *page=daytona2_ppc_page(pc >> 16)) {\n"
                     "  for (UINT32 id=page[(pc & 0xffff) >> 2]; id; id=daytona2_ppc_variants[id].next) {\n"
                     "   const auto &entry=daytona2_ppc_operations[daytona2_ppc_variants[id].operation];\n"
                     "   if (actual == entry.word) { entry.run(); return; }\n  }\n }\n }\n"
                     ' ErrorLog("Untranslated Daytona 2 PowerPC instruction at %08X: %08X", pc, actual);\n'
                     ' ppc.fatalError=true;\n throw std::runtime_error("Untranslated or changed Daytona 2 PowerPC instruction");\n}\n')
    ...
    executor = executor[:switch_start] + "\t\tdaytona2_ppc_fixed(ppc.pc, opcode);" + executor[switch_end:]
    executor = exact_replace(executor, "\treturn executed;", '\tif (ppc.fatalError) throw std::runtime_error("Daytona 2 PowerPC hardware fault");\n\treturn executed;')
...
def main() -> None:
    ...
    manifest = {"format": 1, "cpu": "PowerPC 603R", "upstreamCommit": PIN,
                ...
                "staticRanges": [] if args.trace_only else ["0xffe00000..0xffffffff", "0x00000000..0x001fffff guarded ROM upload alias"]}
