...
def main():
 ...
 rtc='#include <ctime>\nextern std::time_t daytona2_clock();\n'+rtc.replace('\ttime(&currentTime);','\tcurrentTime = daytona2_clock();').replace('localtime(&currentTime)','gmtime(&currentTime)')
 ...
 scsp=scsp.replace(local,'').replace('lastdiff','daytona2_sound_cycle_remainder')
 scsp=scsp.replace(init,'static int daytona2_sound_cycle_remainder = 0;\n\n'+init+'\tdaytona2_sound_cycle_remainder = 0;\n')
 ...
 for relative in ['Src/Graphics/New3D/R3DFrameBuffers.cpp','Src/Graphics/New3D/New3D.cpp','Src/Graphics/Render2D.cpp']:
  ...
  render='extern unsigned daytona2_default_framebuffer();\n'+render.replace('glBindFramebuffer(GL_FRAMEBUFFER, 0);','glBindFramebuffer(GL_FRAMEBUFFER, daytona2_default_framebuffer());').replace('glDrawBuffer(GL_BACK);','glDrawBuffer(GL_COLOR_ATTACHMENT0);')
 ...
 for name,board in [('SoundBoard.cpp',0),('DSB.cpp',1)]:
  ...
  text='extern "C" { extern unsigned daytona2_m68k_board; }\n'+text.replace('M68KSetContext(&M68K);',f'daytona2_m68k_board={board}; M68KSetContext(&M68K);')
 ...
 if kind=='observer':
  ...
  cpu='#include "sound_observe.h"\n'+cpu.replace('REG_IR = m68ki_read_imm_16();','daytona2_sound_observe68k(daytona2_m68k_board, REG_PC, M68KFetch16);\n\t\t\tREG_IR = m68ki_read_imm_16();')
 ...
 sources += [ROOT/'Sources/Bridge/Platform.cpp',ROOT/'Sources/Bridge/daytona2.cpp']
 ...
 if kind!='native':flags+=['-DDAYTONA2_REFERENCE']
 ...
 archive=out/'libdaytona2.a';run(['libtool','-static','-o',archive]+objects,stdout=subprocess.DEVNULL)
 run(['clang++','-dynamiclib','-arch','arm64','-mmacosx-version-min=14.0','-Wl,-dead_strip','-o',out/'libdaytona2.dylib']+objects+['-framework','OpenGL','-framework','Foundation','-lz'])
 ...
 manifest={'kind':kind,'shippingNative':kind=='native','diagnostics':kind!='native','staticArchiveSHA256':sha(archive),'dynamicLibrarySHA256':sha(out/'libdaytona2.dylib'),'upstreamCommit':REVISION,'upstreamArchiveSHA256':upstream_report['archiveSHA256'],'sources':fingerprints,'compiledSources':{str(p.relative_to(ROOT)):sha(p) for p in sources},'headersSHA256':header_digest,'compiler':compiler,'target':'arm64-apple-macos14.0','configuration':'Single-threaded hardware, detached networking, offscreen OpenGL 4.1, stereo downmix, boot-sampled UTC clock'}
 if kind=='native':
