#!/usr/bin/env python3
"""Validate supplied Revision A media and create a deterministic local bundle."""
...
def main():
    ...
    ap.add_argument('--game',type=Path,default=ROOT/'Daytona USA 2 ROMs/daytona2')
    args=ap.parse_args()
    source=ROOT/'build/upstream/supermodel/Config/Games.xml'
    tree=ET.parse(source); game=next(g for g in tree.getroot() if g.get('name')=='daytona2')
    ...
    for region in game.find('roms'):
        for entry in region.findall('file'):
            name=entry.get('name'); data=(args.game/name).read_bytes()
    ...
    manifest={'game':'daytona2','title':'Daytona USA 2 — Battle on the Edge','revision':'Japan Revision A, original country settings available','files':rows}
    ...
    with zipfile.ZipFile(dest,'w',compression=zipfile.ZIP_STORED) as z:
        for row in rows:
            ...
            z.writestr(info,(args.game/row['path']).read_bytes())
