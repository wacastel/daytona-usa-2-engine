...
cd "$(dirname "$0")/.."
MODE=link
ENGINE='build/native/libdaytona2.a'
OUTPUT='build/host/DaytonaUSA2'
while [[ $# -gt 0 ]]; do
    case "$1" in
...
print('Daytona USA 2 host typecheck passed; no engine linked and no app produced.')
REPORT
...
with tempfile.TemporaryDirectory(prefix='fixtures-',dir=directory) as temporary:
    ...
    for marker in ['daytona2_reference_probe_marker','daytona2_diagnostic_engine_marker','daytona2_test_stub_marker',None]:
        ...
        assert not list(scratch.glob('daytona2-host-symbols.*')),name+' leaked its temporary symbol file'
...
GUARD_TEST
    ...
    SYMBOLS="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/daytona2-host-symbols.XXXXXX")"
    ...
    if /usr/bin/grep -E 'daytona2_test_stub_marker|daytona2_reference_probe_marker|daytona2_diagnostic_engine_marker' "$SYMBOLS" >/dev/null; then
