...
manifest=json.loads(path.read_text());archive=Path('build/native/libdaytona2.a')
...
scripts/build_host.sh --engine build/native/libdaytona2.a --output "$APP/Contents/MacOS/DaytonaUSA2"
...
for row in manifest['files']:
    if row['path'] not in {'daytona2.zip','Games.xml','default.nv'}:
...
from pathlib import Path
source=Path(sys.argv[1]);destination=Path('build/Daytona USA 2.app')
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
identity='local.william.daytonausa2'
info=plistlib.loads((source/'Contents/Info.plist').read_bytes())
if info['CFBundleIdentifier']!=identity or info['CFBundleExecutable']!='DaytonaUSA2':
...
report={'app':str(destination),'executableSHA256':sha(destination/'Contents/MacOS/DaytonaUSA2'),
        'engineManifestSHA256':sha(Path('build/native/engine-build-manifest.json')),
        'nativeArchiveSHA256':sha(Path('build/native/libdaytona2.a')),
