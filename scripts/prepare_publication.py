...
Configuration/replays/advanced-auto.json
Configuration/replays/beginner-auto.json
Configuration/replays/beginner-manual.json
Configuration/replays/boot-start.json
Configuration/replays/cabinet-setup.json
Configuration/replays/expert-auto.json
...
Documentation/package-requalification.json
...
def known_media(maximum_source_bytes: int):
    """Hash local supplied media, ZIP/7z members, bundled ZIP, and generated binary images."""
    ...
    supplied = ROOT / 'Daytona USA 2 ROMs'
    ...
    for path in sorted(supplied.rglob('*')):
        if not path.is_file() or path.name == '.DS_Store': continue
        if path.is_symlink(): raise RuntimeError('Media link is outside the scan contract: ' + path.name)
        with path.open('rb') as stream: add(path.relative_to(ROOT).as_posix(), stream, path.stat().st_size)
        if path.suffix.lower() == '.zip':
            with zipfile.ZipFile(path) as archive:
                for member in archive.infolist():
                    if member.is_dir(): continue
                    with archive.open(member) as stream: add(path.name + '/' + member.filename, stream, member.file_size)
            ...
            if any(not line.startswith(('-', 'd')) for line in detail):
                raise RuntimeError('Archive has unsupported links or special files: ' + path.name)
            if any(Path(name).is_absolute() or '..' in Path(name).parts for name in listed):
                raise RuntimeError('Unsafe media archive member: ' + path.name)
                ...
                for member in sorted(Path(work).rglob('*')):
                    ...
                    if member.is_file():
                        with member.open('rb') as stream:
                            add(path.name + '/' + member.relative_to(work).as_posix(), stream, member.stat().st_size)
    bundled = ROOT / 'build/assets/daytona2.zip'
    with bundled.open('rb') as stream: add('bundled/daytona2.zip', stream, bundled.stat().st_size)
...
def scan() -> dict:
    ...
    return {'passed': True, 'files': len(files), 'bytes': sum(p.stat().st_size for p in files),
            ...
            'scope': 'Explicit source allowlist and UTF8 text. Every supplied file and decoded ZIP/7z member, bundled ZIP, and generated .bin/.rom image checked by complete identity and raw/contiguous-hex/base64 encoding when its size could fit. This is a bounded scan, not a general copyright or secret classifier.'}
...
def prepare() -> None:
    ...
    write_json(REVIEW / 'engine-companion-manifest.json', {
        ...
        'publicationActionsPerformed': False, 'upstreamCommit': REVISION,
