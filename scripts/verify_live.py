#!/usr/bin/env python3
"""Run a bounded visible race and validate native display/audio-device telemetry."""
...
def main():
    ...
    binary = ROOT/'build/Daytona USA 2.app/Contents/MacOS/DaytonaUSA2'
    ...
    environment = {**os.environ, 'DAYTONA2_RTC_EPOCH': '946684800'}
    ...
    command = [str(binary), '--audio-replay', str(route), '--sound',
               ...
               '--capture', str(args.output/'race.png'), '--capture-after', str(args.seconds-5),
    ...
    checks = {
        ...
        'frameCaptureWritten': (args.output/'race.png').is_file(),
    ...
    report = {'passed': all(checks.values()), 'checks': checks, 'inputs': identities,
              ...
              'telemetry': result, 'frameCaptureSHA256': sha(args.output/'race.png'),
              'scope': 'Visible AppKit/SpriteKit race and AVFoundation output-device counters. Keep the app focused and run without competing engine workloads. No subjective listening, physical button actuation or physical arcade-board accuracy claim.'}
