...
def main():
 ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--routes',nargs='+',type=Path,required=True);ap.add_argument('--output',type=Path,default=ROOT/'build/validation/integration');args=ap.parse_args()
 args.output.mkdir(parents=True,exist_ok=True)
 native=ROOT/'build/native/libdaytona2.dylib';reference=ROOT/'build/reference/libdaytona2.dylib'
 ...
 for route in args.routes:
  ...
  def replay(kind):
   ...
   env={**os.environ,'DAYTONA2_RTC_EPOCH':'946684800'};env.pop('DAYTONA2_FACTORY_SETTINGS',None)
 ...
 report={'passed':True,'evidenceInputs':evidence,'referenceLibrarySHA256':identities['reference'],'nativeLibrarySHA256':identities['native'],'nativeArchiveSHA256':sha(ROOT/'build/native/libdaytona2.a'),'engineManifestSHA256':sha(ROOT/'build/native/engine-build-manifest.json'),'scriptSHA256':sha(Path(__file__)),'mediaManifestSHA256':sha(ROOT/'build/assets/media-identity.json'),'clockEpoch':946684800,'routes':results,'totalFrames':sum(r['frames'] for r in results),'totalStereoFrames':sum(r['sampleFrames'] for r in results),'scope':'Every RGBA picture, stereo PCM block and block sample count on listed fresh-save routes. Independent processes share a GPU while checking, so elapsed replay time is not a live GUI performance measurement. No physical board or exhaustive gameplay claim.'}
